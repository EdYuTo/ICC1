...
int aux_istrue (int size, char *txt, char *chk, int starttxt, int *startchk, int *end) {

	if (starttxt < size && chk[*end-1] == chk[*startchk+1])
		return aux_istrue(size, txt, chk, starttxt+1, startchk, end);

	else if (txt[starttxt+1] == chk[*end-1]) {
		(*end)--;
		return 1;
	}
...
int istrue (char *txt, char *chk) {
	...
	do {
		if (chk[startchk] == chk[end]) {
			if (aux_istrue(strlen(txt), txt, chk, starttxt, &startchk, &end))
				continue; 
			else
				return 0;
		}
